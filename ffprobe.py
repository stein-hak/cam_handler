...
import os
#from parse_addr import parse_addr



class ffprobe():
    def __init__(self, source, timeout=60, tcp=False,username=None,password=None):


        if username and password:
            self.host, self.proto, self.location, self.username, self.password = parse_addr(source)
            self.source = self.proto + '://' + username + ':' + password + '@' +  self.location.split('//')[1]
        ...
        if tcp:
            com = ['ffprobe', '-hide_banner', '-loglevel', 'fatal', '-show_error', '-rtsp_transport', 'tcp',
                   '-print_format',
                   'json', '-show_format', '-show_streams', self.source]
        else:
            com = ['ffprobe', '-hide_banner', '-loglevel', 'fatal', '-show_error', '-print_format', 'json',
                   '-show_format',
    ...
    def get_fps(self, num_stream=0):
        if self.fps:
            ...
            if stream and 'nb_frames' in stream.keys() and 'duration' in self.format.keys():
                ...
                self.fps = int(round(float(len(self.frames)) / 5.0))
        ...
        if self.frames:
            ...
            if self.is_sane:
                if os.path.isfile(self.source):
                    com = ['ffprobe', '-v', 'quiet', '-print_format', 'json', '-show_frames', '-read_intervals', "%+5",
                           self.source]
    ...
    def estimate_offset(self):
        ...
        # print self.gop
        # print self.fps
        if self.gop is not None and self.fps is not None:
            try:
                sec = self.gop / self.fps

                if sec < 2:
                    return 2
                elif sec > 2:
                    return self.gop / self.fps + 1.5
...
if __name__ == '__main__':
    f = ffprobe(sys.argv[1],username='admin',password='123456')
